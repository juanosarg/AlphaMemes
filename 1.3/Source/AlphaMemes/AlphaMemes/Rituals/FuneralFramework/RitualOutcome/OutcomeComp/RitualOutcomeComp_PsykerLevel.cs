...
{
    public class RitualOutcomeComp_PsykerLevel : RitualOutcomeComp_FuneralFramework
    {
        ...
        {
            int count = assignments.AssignedPawns(roleId).First().GetPsylinkLevel();
            float quality = this.curve.Evaluate((float)count);
            return new ExpectedOutcomeDesc
            {
                label = "AM_RitualRolePsyker".Translate(),
