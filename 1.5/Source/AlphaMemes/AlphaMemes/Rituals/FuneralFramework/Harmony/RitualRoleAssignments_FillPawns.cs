                ...
                foreach(RitualOutcomeComp comp in ritual.outcomeEffect.def.comps)
                {
                    if(comp is RitualOutcomeComp_FuneralFramework)
                    {
                       ...
                       // PsychicRitualRoleDef.Reason reason;
                        Pawn pawn = comp2.BestPawnForRole(__instance.AllCandidatePawns, __instance, out roleid);
                        if(pawn != null)
                        {
                            ...
                            if (!__instance.TryAssign(pawn, __instance.GetRole(roleid),out PsychicRitualRoleDef.Reason reason))
                            {
