...
namespace AlphaMemes
{
    ...
    {
        [HarmonyPostfix]
        public static void Postfix(RitualRoleAssignments __instance, List<Pawn> ___allPawns, Dictionary<string, List<Pawn>> ___assignedRoles)
        {
            if(Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                if (__instance.Ritual.def.HasModExtension<FuneralPreceptExtension>())
                {
                    ...
                    if (___assignedRoles.Values.Any(x=> x.Contains(null)))
                    {
                        string key = ___assignedRoles.FirstOrDefault(x => x.Value.Any(y=>y==null)).Key;
                        ___assignedRoles.Remove(key);
                        List<Pawn> serializablePawnList = new List<Pawn>();
                        serializablePawnList.Add(pawn);
