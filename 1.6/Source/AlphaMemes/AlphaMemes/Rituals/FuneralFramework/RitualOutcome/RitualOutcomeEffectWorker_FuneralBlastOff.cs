...
namespace AlphaMemes
{
    ...
    public class RitualOutcomeEffectWorker_FuneralBlastOff: RitualOutcomeEffectWorker_FuneralFramework
    {
        ...
        public override void ApplyOn(Pawn pawn, Corpse corpse, List<Thing> thingsToSpawn, Dictionary<Pawn, int> totalPresence, LordJob_Ritual jobRitual, RitualOutcomePossibility outcome)
        {
            ...
            float fuel = Mathf.Max(compLaunchable.FuelNeededToLaunchAtDist(compLaunchable.MaxLaunchDistanceAtFuelLevel(compLaunchable.FuelLevel), compLaunchable.parent.Tile.Layer), 1f);
            compTransporter.Launchable.Refuelable.ConsumeFuel(fuel);

            ActiveTransporter innerthing = (ActiveTransporter)ThingMaker.MakeThing(ThingDefOf.ActiveDropPod, null);
            innerthing.Contents = new ActiveTransporterInfo();
        ...
        public List<Settlement> NearbyTradeableSettlements(Thing pod)
        {
            ...
            int maxDistance = compLaunchable.MaxLaunchDistanceAtFuelLevel(compLaunchable.Refuelable.Fuel);
