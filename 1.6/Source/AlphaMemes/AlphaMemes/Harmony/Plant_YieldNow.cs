...
namespace AlphaMemes
{
    ...
    {
        [HarmonyPostfix]
        public static void IncreaseTeaYield(Plant __instance, ref int __result)
        {
            ...
            if ((__instance?.def == InternalDefOf.VBE_Plant_Coffee && Current.Game.World.factionManager.OfPlayer.ideos?.GetPrecept(InternalDefOf.AM_CoffeeYield_Increased) != null)
               )
            {
                __result = GenMath.RoundRandom(__result * 1.1f);
            }
