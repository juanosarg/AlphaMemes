    ...
    public class JobGiver_MergeIntoGaumakerPodCorruptor : ThinkNode_JobGiver
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            foreach (Thing thing in pawn.connections.ConnectedThings)
            {
                ...
                {
                    return JobMaker.MakeJob(InternalDefOf.AM_MergeIntoCorruptedPod, thing, podthing);
                }
            }
            return null;
        }
        ...
        public bool ShouldEnterCorruptedPod(CompTreeConnection compTreeConnection, Pawn dryad)
        {
            ...
            if (compTreeConnection.parent.Map.listerThings.ThingsOfDef(InternalDefOf.AM_CorruptedGaumakerCocoon).Count == 0)
            {
                IntVec3 loc;
                ...
                {
                    podthing = GenSpawn.Spawn(ThingMaker.MakeThing(InternalDefOf.AM_CorruptedGaumakerCocoon, null), loc, compTreeConnection.parent.Map, WipeMode.Vanish);
